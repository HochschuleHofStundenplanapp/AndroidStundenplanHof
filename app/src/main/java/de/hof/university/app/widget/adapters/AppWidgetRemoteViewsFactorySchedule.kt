...
internal class AppWidgetRemoteViewsFactorySchedule(context: Context, lightStyleIsSelected: Boolean)
	...
	override fun setData(dataCache: AppWidgetDataCache): ArrayList<Any> {
		...
		return ArrayList<Any>().apply {
			dataCache.getScheduleData(context).forEach{ i -> i.weekday.also { d ->
	...
	override fun getRemoteView(data: Any): RemoteViews
		= when (data) {
			...
			else -> throw Exception("AppWidgetRemoteViewsFactorySchedule: Unsupported Item in getRemoteView()")
		}
	...
	override fun getRemoteViewForEmptyView(): RemoteViews
		= RemoteViews(packageName, R.layout.widget_list_item_empty).apply {
			setTextViewText(R.id.widget_list_item_empty_message,
					context.getString(R.string.appwidget_list_item_empty_schedule)
			)
